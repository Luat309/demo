...
const genElementsForm = (fields, control, errors) => {
    return fields.map(({ type, ...rest }, index) => {
        switch (type) {
            case "inputNumber":
                return ( <
                    InputNumberController key = { index } {...rest }
                    control = { control }
                    errors = { errors }
                    />
                );
            ...
            case "calender":
                return ( <
                    CalenderController key = { index } {...rest }
                    control = { control }
                    errors = { errors }
                    />
                );
            ...
            case "editor":
                return ( <
                    EditorController key = { index } {...rest }
                    control = { control }
                    errors = { errors }
                    />
                );
            case "select":
                return ( <
                    MultiSelectController key = { index } {...rest }
                    control = { control }
                    errors = { errors }
                    />
                );
            case "dropdown":
                return ( <
                    DropdownController key = { index } {...rest }
                    control = { control }
                    errors = { errors }
                    />
                );
            ...
            default:
                return ( <
                    InputTextController key = { index } {...rest }
                    control = { control }
                    errors = { errors }
                    />
                );
